...
public class AquaFish
{
    ...
    public void changeDir()
    {
     ...
     /** 
      This will check whether or not the fish is at the wall, and will 
      turn the fish around if it is. It will also randomly change the 
      direction of the fish, to make the simulation a little more 
      interesting. Whether or not the fish is at the wall, it moves the 
      fish forward one step.
     **/
    ...
    { //Begin autoChangeDir
        ...
        //The following if loop will change the fish's direction if a random number is less than 3. This will happen regardless of the fish's location.
        if(randomNumber<=2)
        { //Begin random selection for directional change
            randomChangeDir = true;
        } //End random selection for dirrectional change
        //Next, see if the fish is at the wall. If it is, then change its direction.
        if(fishAtWall==true || randomChangeDir==true)
        { //Begin if
            this.changeDir(); //Change the direction of the fish only if it is at the wall
        } //End if
        this.moveForward(); //Move the fish forward.
    } //End autoChangeDir
