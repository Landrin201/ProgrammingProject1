...
public abstract class PercolationController extends SteppedGridAppController
{
    ...
    {
        for(int i=minDensity; i<=maxDensity; i+=increment)
        {
            runNTimes(numRuns, i, currentPercType);
        }
    }
    ...
    public void runNTimes(int numTimes, int density, Class currentPercType)
    {
        ...
        {
            this.randomlyFillGrid(density, currentPercType);
            this.invisibleRun();
            if(percolatedToBottom()==true)
            {
                count++;
            }
        }
        System.out.println("Size of Grid: " + width + "x" + height + "\n"
                            + "Density: " + density + "\n"
                            + "Number of runs: " + numTimes + "\n"
                            + "Number Percolated: " + count + "\n");
    }
